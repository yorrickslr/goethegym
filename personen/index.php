<?php
		...
		<body>
			<header class="noselect">
				...
				<nav>
					...
					<a href="../personen/" title="Personen" class="current material-icon">&#xE7EF;</a>
			...
			<?php
				if($path["filename"]=="index" || $path["filename"]=="") {
					$articles = simplexml_load_file("../temp/personen/index.xml");
					?>
					<h2>Personen</h2>
					<nav>
					<?php
						...
						echo "</nav>\n<article>";
						foreach($articles->article as $article) {
							$url = "../temp/personen/" . $article->path . ".html";
	...
	if($path["extension"] == "html") {
		$url = "../temp/personen/" . $path["filename"] . ".html";
		$file = file_get_contents($url);
		if($file == FALSE) {
			http_response_code(404);
			header("Location: /errorpag/404.html");
